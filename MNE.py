...
def get_G_from_edges(edges):
    ...
    for edge_key in edge_dict:
        ...
        tmp_G.add_edge(edge_key.split(' ')[0], edge_key.split(' ')[1])
        tmp_G[edge_key.split(' ')[0]][edge_key.split(' ')[1]]['weight'] = weight
    return tmp_G
...
def load_single_network_data(f_name):
    # This function is used to load multiplex data
    print('We are loading single_view data from:', f_name)
    node_2_id = dict()
    all_edges = list()
    all_nodes = list()
    idx=0
    with open(f_name, 'r') as f:
        for line in f:
            words = line[:-1].split('\t')
            if words[0] not in node_2_id:
                node_2_id[words[0]] = idx
                idx+=1
            if words[1] not in node_2_id:
                node_2_id[words[1]] = idx
                idx+=1
            all_edges.append((node_2_id[words[0]], node_2_id[words[1]],words[2]))
    print('Finish loading data')
    return node_2_id, all_edges
...
def train_model(network_data):
    ...
    print('finish random walk on the graph')
    base_embedding, _, _, index2word = train_embedding(None, base_walks, 'Base', 100, 10, 1)
    print('finish initial node embedding on the graph')
    final_model = dict()
    ...
    for layer_id in network_data:
        ...
        print('finish temp node embedding on the graph')
        ...
        print('finish final node embedding on the graph')
    return final_model
