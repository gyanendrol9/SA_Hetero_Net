...
class hetero_multilayer:
    ...
    def compute_transition_matrix(self):
        ...
        if compute_weights:
            ...
            for key in self.multiplexes.keys():
                delta[key] = round(self.multiplexes[key].layers[0].edge_df.shape[0]/ total_edges, 4)
        ...
        if (len(sort_keys)) == 1:
            ...
            self.supra_transition_matrix[0:n1, 0:n1] = delta[k1] * self.supra_transition_matrix[0:n1, 0:n1]
            if k1+"-"+k2 in self.bipartite_matrix :
                if self.bipartite_matrix[k1+"-"+k2] is not None:
                    self.supra_transition_matrix[0:n1, n1:n1 + n2] = (1.0 - delta[k1]) / (
            ...
            if k2 + "-" + k1 in self.bipartite_matrix:
                if self.bipartite_matrix[k2+"-"+k1] is not None:
                    self.supra_transition_matrix[n1:n1+n2, 0:n1] = (1.0-delta[k2])/(L-1) * 1.0 * self.supra_transition_matrix[n1:n1+n2, 0:n1]
            self.supra_transition_matrix[n1:n1+n2, n1:n1+n2] = delta[k2] * self.supra_transition_matrix[n1:n1+n2, n1:n1+n2]
            ...
            if k1 + "-" + k3 in self.bipartite_matrix:
                if self.bipartite_matrix[k1+"-"+k3] is not None:
                    self.supra_transition_matrix[0:n1, n1+n2 : n1+n2+n3] = (1.0 - delta[k1]) / (
            ...
            if k2 + "-" + k3 in self.bipartite_matrix:
                if self.bipartite_matrix[k2 + "-" + k3] is not None:
                    self.supra_transition_matrix[n1:n1 + n2, n1 + n2:n1 + n2 + n3] = (1.0-delta[k2])/(L-1) * 1.0 * self.supra_transition_matrix[n1:n1 + n2, n1 + n2:n1 + n2 + n3]

            if k3 + "-" + k1 in self.bipartite_matrix:
                if self.bipartite_matrix[k3 + "-" + k1] is not None:
                    self.supra_transition_matrix[n1+n2:n1+n2+n3, 0:n1] = (1.0-delta[k3])/(L-1) * 1.0 * self.supra_transition_matrix[n1+n2:n1+n2+n3, 0:n1]
            if k3 + "-" + k2 in self.bipartite_matrix:
                if self.bipartite_matrix[k3 + "-" + k2] is not None:
                    self.supra_transition_matrix[n1+n2:n1+n2+n3, n1:n1 + n2] = (1.0-delta[k3])/(L-1) * 1.0 * self.supra_transition_matrix[n1+n2:n1+n2+n3, n1:n1 + n2]
            self.supra_transition_matrix[n1+n2:n1+n2+n3, n1+n2:n1+n2+n3] = delta[k3] * self.supra_transition_matrix[n1+n2:n1+n2+n3, n1+n2:n1+n2+n3]
